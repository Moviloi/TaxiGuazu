...
> **v3.12 — 2026-07-05** · Revisión arquitectónica post-cierre 37 tareas AITOS: GAP-08 a GAP-12, DEBT-13 y Launch Readiness Checklist registrados en sección H. Contexto crítico: sistema NUNCA corrió con tráfico real de producción. Commit `8fc6a3f` + push.
>
> **v3.11 — 2026-07-05** · AIT-051 completado: `GET /api/bot/metrics` endpoint creado en `src/app/api/bot/metrics/route.ts`, protegido con `checkAdminAuth()` (ADMIN_API_KEY). 6 métricas en response JSON: active_conversations_24h, completed_trips_7d, conversion_rate_7d, avg_response_time_7d, escalation_rate_7d, escalation_reason_count. Queries real-time contra `conversations`, `trips`, `messages`, `chat_sessions`. Ventanas temporales consistentes: 24h para actividad inmediata, 7d para el resto. Métrica 2 y 3 anotadas como TEMPORAL — simplificar a `trip_events` cuando todos los trips tengan eventos (post AIT-042). Build clean, 771 tests (60 files), 0 regresiones. AEL enforce PASS.
> **v3.10 — 2026-07-05** · AIT-052 completado: Sistema de evals creado — dataset de 25 casos (`scripts/eval-cases/dataset.ts`) con 5 grupos obligatorios (customs EN, frontera PT, frustración real, falsos positivos de frustración, ambigüedad genuina) + 15 casos regulares (booking ES/EN/PT, NOW, greeting, commercial, consulta, emergency, reschedule, post-service). Runner `scripts/run-evals.ts` que ejecuta `core()` + `router()` + FRUSTRATION_RE contra cada caso y reporta precisión por intent, output_type, slots, frustración y escalación. `npm run evals` script agregado. **Resultado: 24/25 PASS (96%)** — único fallo es falso positivo conocido de FRUSTRATION_RE con "leé bien" en contexto de dirección. Gaps documentados: core() sin NLU EN/PT, POST_SERVICE no detectado, parsing "del X al Y" fusiona slots, frustración + extracción sin origin/destination. Build clean, 771 tests, 60 files, 0 regresiones. AEL enforce PASS.
> **v3.9 — 2026-07-05** · AIT-050 completado: `@sentry/nextjs@10.63.0` instalado, `instrumentation.ts` con Sentry.init condicional (DSN opcional — no rompe sin él), `instrumentation-client.ts` con hooks de navegación, `next.config.ts` envuelto con `withSentryConfig` silencioso sin DSN. Webhook (`/api/whatsapp/webhook`) envuelto con `wrapRouteHandlerWithSentry`. Endpoint `/api/sentry-test` creado para verificación. Warnings de instrumentation suprimidos. Build clean (0 warnings, 0 errors). 771 tests, 0 regresiones. AEL enforce PASS. **Requiere `SENTRY_DSN` provisto por Cristian desde Sentry.io para activar captura en producción.**
> **v3.8 — 2026-07-05** · AIT-043 completado: los 6 eventos de Dispatch (Initiated, Offered, Broadcasted, Accepted, Abandoned, Contingency) se escriben en `dispatch_events`. PASOS A-C: `advanceToNivel1/2/3` y `advanceToWaitingDriver` inyectan evento con `transitionTo()`. PASO D: `closeWorkflow()` recibe `eventType` REQUERIDO — 5 callers actualizados (4 en dispatch.service.ts, 1 en timeouts.ts). PASO E: `assignWorkflowAtomic(phone, driverPhone)` lee estado actual, hace UPDATE atómico, y si hay trip activo, inserta `DispatchAccepted` con level y actorPhone=driverPhone. R2 limpiado: `insertDispatchEvent()` movida de services/ a `src/lib/db/domains/dispatch-events.ts`. Build OK, 771 tests (60 files), 0 regresiones. AEL enforce PASS. GAP-06 documentado (DispatchOffered no cubre broadcast).
> **v3.7 — 2026-07-04** · AIT-042 completado: los 5 eventos de trip se escriben hoy en `trip_events`. TripCreated (`createTrip`) y TripDriverAssigned (`assignDriverToTrip`) con payload rico y executor opcional. TripReconfirmed / TripCompleted / TripCancelled centralizados en `updateTripState()` mediante mapa `STATUS_TO_TRIP_EVENT` (payload null). `completeTrip()` es el único fuera del mapa (payload rico commission/payout/price). GAP-05 actualizado con cobertura global. Build OK, 761 tests, 0 regresiones.
...
| DEBT-02 | Dependencia survey → lead (NO circular, cadena lineal) | services/ | P1 | `DONE` ✅ | Archivo lead-event-helpers.ts eliminado en T3. Sin consumidores restantes. Dependencia resuelta. |
...
| **DEBT-12** | **`connection.ts initSchema()` drift de schema contra DB real Turso** | `src/lib/db/core/connection.ts` | **P0** | **`DONE` ✅** | **Fase C1 (DONE):** initSchema() reemplazado con DDL completo de 44 tablas. Verificación de paridad contra Turso: 100% — 44/44 tablas, 0 diferencias de columnas. `npm run build` OK. 740 tests, 0 regresiones. AEL enforce PASS.<br><br>**Fase C2 (DONE):** Migration runner real implementado. `scripts/run-migrations.ts`, `db/migrations/`, `npm run migrate`. 13 migraciones históricas seedeadas en `_migrations` vía `npm run migrate:seed-history`. Probado con migración de prueba: APPLIED → idempotente → SKIPPED ✅<br><br>**Fase C3 (DONE):** 7 bloques ALTER TABLE inline (36 statements) eliminados de `initSchema()` en `connection.ts`. Cada columna verificada contra Turso real (37/37 OK). DB nueva desde cero genera schema completo sin ALTER TABLE. validate-schema-parity: 44/44 OK, 0 drift.<br><br>**Estado post-C3:** `connection.ts` ya no contiene ALTER TABLE inline. Todo el schema histórico vive en `initSchema()` como CREATE TABLE completo (C1). Todo cambio futuro pasa por `db/migrations/` (C2).<br><br>**[Descubierto durante verificación AIT-040 contra DB real Turso — 2026-07-04]** |
| GAP-01 | policy-pipeline.ts no valida disponibilidad de flota antes de dispatch | workflow/policy-pipeline.ts | P1/P2 (a definir) | `DEFERRED` | El chequeo de flota ocurre hoy en trip-execution.service.ts y now-execution.service.ts, más tarde en el flujo, y ya frena la ejecución cuando no hay capacidad. Moverlo a fail-fast en el pipeline es mejora de UX, no fix de bug. Revisar solo si hay evidencia real de impacto en usuarios. |
| GAP-02 | dispatchTool.dispatch() validado, pero executeDispatch sigue siendo consumido directamente por trip-execution.service.ts y now-execution.service.ts | workflow/policy-pipeline.ts | P1/P2 (a definir) | `DEFERRED` | dispatchTool.dispatch() (AIT-022) ya está validado con contrato Zod y tests de equivalencia. El consumo real de executeDispatch hoy está en trip-execution.service.ts y now-execution.service.ts, más tarde en el flujo, y ya funciona correctamente. Migrar esos consumos a dispatchTool.dispatch() es mejora de consistencia interna, no fix de bug. Revisar solo si hay evidencia real de impacto en usuarios. |
...
| GAP-06 | **DispatchOffered no cubre ofertas en broadcast** | `src/lib/services/dispatch/dispatch.service.ts` | P2 (candidato) | `DEFERRED` | DispatchBroadcasted ya registra que el broadcast ocurrió; desglose por oferta individual es mejora de granularidad, no gap funcional. Revisar solo si se necesita rastrear quién recibió/declinó cada oferta individual dentro de un broadcast. |
| GAP-07 | **Falso positivo FRUSTRATION_RE: alternativa #13 `le[ée]\s+bien` captura énfasis instructivo como frustración** | `data/knowledge/policies/escalation.json` (frustrationPattern) | P2 (candidato) | `NEEDS VALIDATION` | **Descubierto durante AIT-052 eval (FRUST_FP_02).** La alternativa #13 del regex FRUSTRATION_RE (`le[ée]\s+bien`) matchea frases como *"Leé bien la dirección que te pasé, es Av. Victoria Aguirre 123"* donde el usuario no está frustrado — está dando una dirección con énfasis instructivo. En español rioplatense "leé bien X" es una fórmula conversacional común para reforzar información, no necesariamente una queja.<br><br>**La alternativa #12 (`no\s+le[ée]s`) YA cubre el caso genuino de frustración** (*"no leés bien"* = queja directa sobre la capacidad del bot). La #13 agrega falsos positivos sin cubrir un caso real que #12 no cubra.<br><br>**Solución candidata:** Eliminar la alternativa #13 del regex o acotarla para que solo matchée cuando NO va seguida de información concreta (ej: que aparezca sola, sin complemento directo).<br><br>**No implementar sin decisión explícita** — cambiar FRUSTRATION_RE en producción requiere validación del impacto en tasas de escalación actuales. |
...
*Estos fixes corren en paralelo a las etapas de modelado. Riesgo de producción activo hoy — no esperan a que termine ninguna fase de documentación.*
...
| **AIT-004** | **Hacer WHATSAPP_APP_SECRET obligatorio para HMAC verification** | P0-paralelo | Ninguna | `src/app/api/whatsapp/webhook/route.ts`, `src/config/env.ts` | 1. `env.ts`: `WHATSAPP_APP_SECRET` migrado de opcional a `z.string().min(1, ...)` requerido en schema Zod. 2. `route.ts`: `verifySignature()` cambiado — sin secret retorna `false` en vez de `true` (antes salteaba verificación). 3. `postHandler()`: sin firma válida → 401. | Webhook sin HMAC válido retorna 401. Servidor arranca sin `WHATSAPP_APP_SECRET` (getAppSecret() captura error de env Zod y retorna null), pero cada POST falla con 401. HMAC hardening completo en commit `3fa18b0`. | ✅ DONE |
| **AIT-005** | **Implementar rate limiting en webhook por número de teléfono** | P0-paralelo | Ninguna | `src/app/api/whatsapp/webhook/route.ts`, `src/lib/db/database.ts` (`getConnectionValue`, `setConnectionValue`) | 1. `checkRateLimit()` implementada: key `rate_limit_{phone}`, almacena array de timestamos como JSON en `connection_state`. 2. Ventana deslizante: filtra timestamos con `now - ts < 60_000`. 3. Si `valid.length >= 10` → retorna `false` → POST handler retorna 429. 4. Rate limit chequeado antes de cualquier procesamiento, después de HMAC. | Un número enviando >10 mensajes en 60s recibe 429. Mensajes legítimos no se ven afectados. Implementación completa en commit `3fa18b0`. | ✅ DONE |
...
| **AIT-042** | **Implementar event logger: escribir eventos en cada transición de Trip** | P2-events | AIT-040 | `src/lib/db/domains/trips.ts` | 1. `insertTripEvent()` + `createTrip()` (TripCreated, payload rico) [PASO 1]. 2. `assignDriverToTrip()` (TripDriverAssigned, payload rico) [PASO 2]. 3. Mapa `STATUS_TO_TRIP_EVENT` en `updateTripState()` cubre TripReconfirmed, TripCompleted, TripCancelled con payload null en un solo punto [PASOS A-C]. 4. `completeTrip()` (TripCompleted, payload rico) es el único fuera del mapa [PASO D]. 5. GAP-05 documenta best-effort con executor opcional. | Los 5 eventos de Trip se escriben en `trip_events` en cada transición. `SELECT * FROM trip_events WHERE trip_id = ?` reconstruye la historia completa. build OK, 761 tests, 0 regresiones. | **DONE** ✅ |
| **AIT-043** | **Implementar event logger: escribir eventos en cada transición de Dispatch** | P2-events | AIT-041 | NUEVO: `src/lib/db/domains/dispatch-events.ts`. Modificados: `src/lib/services/dispatch/dispatch-workflow.ts`, `src/lib/services/dispatch/dispatch.service.ts`, `src/lib/services/dispatch/driver.service.ts`, `src/lib/db/domains/trips.ts`, `src/lib/timeouts.ts` | **PASO A:** `advanceToNivel1()` y `advanceToWaitingDriver()` inyectan `DispatchInitiated` con nivel = newState via `transitionTo()`.<br>**PASO B:** `advanceToNivel2()` inyecta `DispatchOffered` con nivel = newState.<br>**PASO C:** `advanceToNivel3()` inyecta `DispatchBroadcasted` con nivel = newState.<br>**PASO D:** `closeWorkflow(convId, eventType)` con `eventType` REQUERIDO (DispatchAbandoned o DispatchContingency). 5 callers actualizados (4 en dispatch.service.ts, 1 en timeouts.ts). `transitionTo()` recibe `eventType` como parámetro obligatorio.<br>**PASO E:** `assignWorkflowAtomic(phone, driverPhone)` — lee estado actual vía `queryOne`, hace UPDATE atómico, y si hay trip activo (via `getActiveTripByPhone`), inserta `DispatchAccepted` con level = currentState y actorPhone = driverPhone.<br><br>**R2 resuelto:** `insertDispatchEvent()` vive en `src/lib/db/domains/dispatch-events.ts` (infraestructura BD), no en services/. `dispatch-workflow.ts` ya no tiene `getDb()` directo.<br><br>**GAP-06:** `DispatchOffered` no cubre ofertas en broadcast (solo en `advanceToNivel2`). Documentado sin implementar. | 6 eventos de Dispatch se escriben en `dispatch_events` en cada transición: Initiated, Offered, Broadcasted, Accepted, Abandoned, Contingency. `SELECT * FROM dispatch_events WHERE trip_id = ?` reconstruye la traza completa. 60 files, 771 tests, 0 regresiones. AEL enforce PASS. | **DONE** ✅ |
...
| ID | Título | Fase | Dependencias | Archivos reales | Pasos técnicos | Criterio de aceptación | Estado |
|----|--------|------|-------------|-----------------|-----------------|----------------------|--------|
| **AIT-050** | **Integrar Sentry para captura de errores en producción** | P2-obs | Ninguna | `instrumentation.ts`, `instrumentation-client.ts`, `next.config.ts`, `src/app/api/whatsapp/webhook/route.ts` | 1. `@sentry/nextjs@10.63.0` instalado con `--legacy-peer-deps`. 2. `instrumentation.ts` creado con Sentry.init condicional (DSN opcional — modo silencioso si no hay DSN). 3. `instrumentation-client.ts` creado (cliente, con `onRouterTransitionStart`). 4. `next.config.ts` envuelto con `withSentryConfig` (silent sin DSN). 5. Webhook envuelto con `wrapRouteHandlerWithSentry`. 6. `/api/sentry-test` endpoint de verificación. 7. `sentry.server.config.ts` y `sentry.edge.config.ts` eliminados (App Router migration). | Build clean sin DSN. Servidor arranca sin DSN. Sentry captura errores silenciosamente cuando no hay DSN configurado. Cuando Cristian provea DSN, errores aparecen en dashboard de Sentry. | ✅ DONE |
| **AIT-051** | **Métricas básicas de negocio: endpoint de KPIs** | P2-obs | Ninguna | `src/app/api/bot/metrics/route.ts`, `src/lib/auth.ts` (checkAdminAuth), tablas `conversations`, `trips`, `messages`, `chat_sessions` | 1. Endpoint `GET /api/bot/metrics` protegido con `checkAdminAuth` (ADMIN_API_KEY via header `x-api-key`). 2. 6 métricas: active_conversations_24h, completed_trips_7d (TEMPORAL: desde trips.trip_phase — migrar a trip_events cuando todos los trips tengan eventos), conversion_rate_7d (TEMPORAL: misma nota), avg_response_time_7d, escalation_rate_7d (mode='HUMAN'), escalation_reason_count (chat_sessions.escalation_reason, con try/catch por schema desactualizado). 3. Queries en paralelo contra DB, sin lógica de negocio adicional. 4. avg_response_time calculado como diferencia m1.created_at - m2.created_at entre pares user→assistant en últimos 7d. | `GET /api/bot/metrics` con x-api-key válida retorna JSON con 6 métricas. Build: 0 errores, 0 warnings. Tests: 771/771 (60 files). enforce.sh PASS. 0 `as any`. | ✅ DONE |
| **AIT-052** | **Sistema de evals: evaluar calidad de respuestas del bot** | P2-obs | Ninguna | `scripts/eval-cases/dataset.ts`, `scripts/run-evals.ts`, `package.json` (`"evals"` script) | 1. Dataset de 25 casos creado en `scripts/eval-cases/dataset.ts` con 5 grupos obligatorios (CUSTOMS_EN, FRONTERA_PT, FRUST_TRUE/FP, AMBIGUOUS) + 15 regulares. 2. Runner `scripts/run-evals.ts` ejecuta `core()` + `router()` + FRUSTRATION_RE contra cada caso. 3. Reporta tabla por caso, resumen agregado (precisión intent 100%, output_type 100%, slot recall 90%, frustration 96%), y breakdown por tag. 4. Gaps conocidos documentados: core() sin NLU EN/PT, POST_SERVICE sin detectar, parsing "del X al Y" fusiona slots. | `npm run evals` ejecuta dataset de 25 casos y reporta métricas. **Resultado: 24/25 PASS (96%).** Único fallo: falso positivo conocido FRUSTRATION_RE con "leé bien". 771 tests, 60 files, 0 regresiones. Build clean. AEL enforce PASS. | ✅ DONE |
...
| ID | Título | Fase | Dependencias | Archivos reales | Pasos técnicos | Criterio de aceptación | Estado |
|----|--------|------|-------------|-----------------|-----------------|----------------------|--------|
| **AIT-060** | **Inferencia de aeropuerto: detectar aeropuerto probable sin preguntar** | P3-oi | AIT-030 | `src/lib/services/extraction/entity-extractor.ts`, `src/lib/services/extraction/confidence.ts`, `src/lib/ai/slot-state.ts`, `src/lib/ai/slot-confirmation.ts`, `src/lib/ai/extraction-schema.ts` | 1. `entity-extractor.ts`: `AIRPORT_CODE_RE` detecta IGR/IGU/AGT en texto del usuario, resuelve vía DB a nombre canónico + setea `airport_code`. 2. `confidence.ts`: SlotKey extendido con `airport_code`; códigos explícitos obtienen score 1.0, reason "explicit". 3. `slot-state.ts`: bloque `airport_code` — explícito → CONFIRMED/USER_PROVIDED, inferido → CONFIRMATION_PENDING/SYSTEM_INFERRED. 4. `slot-confirmation.ts`: integración de `airport_code` en UI de confirmación (shouldRequestConfirmation + buildSlotConfirmationMessage). 5. 5 tests de slot-state. | Al escribir "IGR a las 10", el bot reconoce el código, lo resuelve a "Aeropuerto IGR (Cataratas del Iguazú)" con score 1.0 y lo marca CONFIRMED sin preguntar. Si el sistema infiere IGR por contexto (score 0.8, reason "inferred"), lo presenta como sugerencia con CONFIRMATION_PENDING. Commits: `0cf3179` (2026-07-05). Note: `airport-inference.ts` (177 líneas) era código muerto — ningún archivo de producción lo importaba. Se eliminó junto con su test. Los tests de buildSlotStates se migraron a `tests/ai/slot-state-airport-code.test.ts`. | **DONE** |
| **AIT-061** | **Inferencia de horario: deducir hora probable según tipo de viaje** | P3-oi | AIT-032 | `src/lib/services/extraction/time-inference.ts`, `src/lib/ai/slot-state.ts`, `src/lib/services/extraction/extraction-runner.ts`, `src/lib/services/i18n/catalog.ts` | 1. Motor `inferPickupTime()` con árbol de 3 pasos (usuario dio hora → explicit; destino en ATTRACTION_PICKUP → inferred; otro → none). 2. `slot-state.ts`: bloque `inferred_opening_hours` ANTES de preserve prev (para que preserve pueda restaurar CONFIRMED si el valor ya fue confirmado). 3. `extraction-runner.ts`: conexión post-pricing, pre-prev-slots. 4. `catalog.ts`: claves `time.suggest` y `time.ask` en 3 idiomas. 5. 38 tests (34 unit + 4 integración real con pipeline REAL: `calculateSlotConfidence()` → `inferPickupTime()` → `buildSlotStates()`). Verifica que "mañana quiero ir al Parque Nacional Iguazú" produce `scheduled_at` con reason `inferred_opening_hours` y status `CONFIRMATION_PENDING`. | Al escribir "tomorrow to the falls", el bot responde "The park opens at 8am. Shall I schedule pickup for 7:45am?" con botones [Yes, 7:45] [Change time]. Commits: `6261e28` (2026-07-05). 814 tests, 63 files, 0 regresiones. Build PASS. enforce PASS. | **DONE** |
...
<!-- ═══════════════════════════════════════════════════════════════════════ -->
<!-- SECCIÓN H: HALLAZGOS DE REVISIÓN ARQUITECTÓNICA POST-AITOS (2026-07-05)   -->
<!-- ═══════════════════════════════════════════════════════════════════════ -->

## H — Hallazgos de revisión arquitectónica post-AITOS

*Surgidos de revisión arquitectónica conceptual posterior al cierre de las 37 tareas del pipeline AITOS. Ordenados por impacto funcional real (mayor a menor).*

> **CONTEXTO CRÍTICO:** El bot / AI Transportation OS NUNCA corrió con tráfico real de producción. TaxiGuazú como negocio está activo (taxiguazu.com.ar operativo), pero hoy el flujo real es sitio web → WhatsApp → chat directo con el fundador, sin IA de por medio. Todo trip/evento en Turso hasta la fecha es dato de desarrollo/prueba, no de cliente real. Este sistema es, literalmente, versión cero: cero usuarios reales atendidos hasta hoy. Esto reduce el riesgo de privacidad/retención de datos reales, pero eleva la importancia de un checklist de primera puesta en producción antes de conectar tráfico real por primera vez.

| ID | Título | Componente | Prioridad | Estado | Notas |
|----|--------|-----------|-----------|--------|-------|
| **GAP-10** | **Condicionantes de riesgo en itinerarios multi-tramo + confirmación no-absoluta con criterio del chofer** | Sistema de viajes / multi-ride | ALTO impacto funcional | `NEEDS VALIDATION` | El sistema no modela tiempos estimados por tramo/servicio, ni condicionantes de riesgo para confirmar itinerarios multi-ride (demora impredecible en aduana, clima para excursión de luna llena). Tampoco existe una modalidad de confirmación donde el chofer, con criterio de campo en tiempo real, proponga un ajuste (ej: adelantar la salida) que dispare su propio circuito de confirmación hacia el pasajero — distinto del patrón "sistema infiere, pasajero confirma" ya construido en P3-OI. Es una capacidad nueva, no una extensión de lo existente. Requiere definición de producto antes de diseño técnico. |
| **GAP-09** | **Ausencia de "Agenda" como capa de seguimiento prospectivo** | Sistema de viajes / programación | ALTO impacto funcional | `NEEDS VALIDATION` | Cron (ejecución programada) y Calendario (datos de referencia: feriados, lunas llenas) existen. Agenda —vista agregada de servicios comprometidos a futuro con seguimiento activo— no. Hoy cada viaje vive aislado en su fila; el único seguimiento proactivo real es la reconfirmación 24hs antes (reconfirmado_24hs). No hay vista tipo "servicios comprometidos próximos 7 días con estado de preparación". Requiere definición de alcance de producto. |
| **Launch Readiness** | **Checklist de primera puesta en producción** | Sistema / despliegue | ALTO impacto práctico | `NEEDS VALIDATION` | Antes de conectar el bot a tráfico real de producción por primera vez, armar y ejecutar un checklist explícito que incluya al menos: activar Sentry con DSN real (hoy integrado pero inactivo), definir plan de backup/restore de datos reales (no solo de schema — eso ya lo cubre DEBT-12), prueba de carga/concurrencia mínima dado el riesgo conocido de cold starts serverless en los timers de Dispatch, y checklist de rollback si algo falla en las primeras horas reales. Requiere que Cristian defina fecha objetivo de lanzamiento real. |
| **GAP-08** | **Bounded context de "toma del viaje" (pre-Dispatch) sin formalizar** | policy-pipeline.ts / dominio Session/Trip | MEDIO impacto funcional/estructural | `NEEDS VALIDATION` | Lo que hoy se llama informalmente "reserva" está repartido entre política conversacional (policy-reserva.ts, dominio Session) y creación de Trip (dominio Trip). Dispatch es un motor separado y correctamente delimitado. No hay bounded context propio para "todo lo que ocurre antes de que el Trip exista". Riesgo: ambigüedad de responsabilidad en futuras features que toquen ese tramo. |
| **DEBT-13** | **Nombres de tabla en producción con etiquetas de fase de desarrollo** | DB / naming | BAJO impacto funcional, ALTO impacto de higiene/mantenibilidad | `DEFERRED` | f9_events, f9_drift_log, f9_error_log, conversation_f4_log llevan etiquetas de fase de desarrollo grabadas en el nombre real de la tabla en Turso — mismo antipatrón que prohibimos para código nuevo, pero preexistente y nunca auditado. Renombrar tablas reales con datos requiere su propia migración cuidadosa (mismo mecanismo de DEBT-12 C2), no un find-replace. No bloquea funcionamiento; priorizar después de los GAP de impacto funcional alto. |
| **GAP-11** | **Documentación visual incompleta** | docs/diagrams | BAJO impacto funcional directo, ALTO valor para onboarding de terceros | `DEFERRED` | Los diagramas existentes describen el sistema pre-sesión; nada de lo construido hoy (tools, event sourcing, knowledge layer, P3-OI, migration runner, ADR-006) está diagramado. Faltan tipos completos: DFD por niveles, diagrama de despliegue (topología real: Vercel, Turso, Meta Cloud API, Sentry, cron), diagrama de entidades completo (44 tablas). |
| **GAP-12** | **Auditoría de organización de archivos/tests/scripts** | Varios | BAJO impacto funcional directo, reduce riesgo de duplicación futura | `DEFERRED` | No se hizo una auditoría exhaustiva de duplicados/archivos huérfanos/convención de carpetas de tests (tests/services, ai, tools, unit, integration sin taxonomía explícita) ni de scripts/ (mezcla de one-shots históricos, validadores, runners sin agrupar). |
