...
| **AIT-031** | **Extraer conocimiento operacional a data/knowledge/ops/** | P1-knowledge | AIT-030 | `src/lib/ai/taxiguazu-knowledge.ts` L1-121, `src/lib/ai/iguazu-knowledge.ts` (migration, transport, practical, calendar), NUEVO: `data/knowledge/ops/operations.json`, `data/knowledge/ops/migration.json`, `data/knowledge/ops/pricing-rules.json` | 1. Extraer `TAXIGUAZU_KNOWLEDGE` (coordination, borderCrossing, pricing, tripTypes, communication) a `operations.json`. 2. Extraer `migration` (docs, QR, eVisa) a `migration.json`. 3. Extraer reglas de pricing no capturadas en tariffs DB a `pricing-rules.json`. | Conocimiento operacional versionado y editable. `getOperationalInfoPrompt()` sigue funcionando. | PENDIENTE |
| **AIT-032** | **Extraer conocimiento comercial: festivos, temporadas, surges** | P1-knowledge | AIT-030, AIT-031 | `src/lib/ai/iguazu-knowledge.ts` (calendar section), `src/lib/ai/taxiguazu-knowledge.ts`, NUEVO: `data/knowledge/commercial/calendar.json`, `data/knowledge/commercial/surge-rules.json` | 1. Extraer `calendar` (Luna Llena, holidays AR/BR/PY, seasons) a `calendar.json`. 2. Definir reglas de surge pricing (festivos +X%, temporada alta +Y%) en `surge-rules.json`. 3. Conectar con pricing engine para aplicar surges automáticamente. | El pricing engine aplica surge en festivos automáticamente. Agregar un nuevo feriado es una entrada JSON. | PENDIENTE |
| **AIT-033** | **Extraer políticas de negocio a data/knowledge/policies/** | P1-knowledge | Ninguna | `src/lib/ai/policy-reserva.ts`, `src/lib/ai/policy-ahora.ts`, NUEVO: `data/knowledge/policies/reserva.json`, `data/knowledge/policies/ahora.json`, `data/knowledge/policies/escalation.json` | 1. Extraer reglas de decisión de `policy-reserva.ts` (10 niveles de prioridad) a `reserva.json`. 2. Extraer reglas de `policy-ahora.ts` a `ahora.json`. 3. Documentar reglas de escalación en `escalation.json`. 4. Las policies siguen siendo código TypeScript, pero las reglas son datos. | Cambiar una regla de negocio (ej: "si el precio es >X, pedir confirmación extra") es editar un JSON, no recompilar TypeScript. | PENDIENTE |
| **AIT-034** | **Sistema de versionado de knowledge layer** | P1-knowledge | AIT-030, AIT-031, AIT-032, AIT-033 | NUEVO: `data/knowledge/manifest.json`, `scripts/validate-knowledge.ts` | 1. Crear `manifest.json` con versión semántica del knowledge layer, hash de cada archivo, y timestamp. 2. Crear script `validate-knowledge.ts` que verifique integridad de todos los archivos JSON (schemas, referencias cruzadas, sin duplicados). 3. Agregar al pre-commit hook. | `npm run validate-knowledge` verifica que el knowledge layer es consistente. El manifest se actualiza automáticamente al modificar cualquier archivo. | PENDIENTE |
...
| ID | Título | Fase | Dependencias | Archivos reales | Pasos técnicos | Criterio de aceptación | Estado |
|----|--------|------|-------------|-----------------|-----------------|----------------------|--------|
| **AIT-040** | **Diseñar schema de eventos para Trip** | P2-events | AIT-010 | NUEVO: `src/lib/db/domains/trip-events.ts`, tabla `trip_events` en `src/lib/db/core/connection.ts` | 1. Definir tabla `trip_events`: `id, trip_id, event_type (TripCreated, TripQuoted, TripConfirmed, TripAssigned, TripStarted, TripCompleted, TripCancelled), payload JSON, occurred_at, actor (system/user/driver)`. 2. Agregar DDL en `initSchema()`. 3. Tipos en `db/types.ts`. | Tabla `trip_events` existe en schema. Insertar un evento no modifica el estado del viaje (eventos son append-only). | PENDIENTE |
